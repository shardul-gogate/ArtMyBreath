	...
	private fun registerUser() {
		...
		firebaseAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener {
			if (it.isSuccessful) {
				...
				USER_COLLECTION_REFERENCE.document(userID).set(profileMap)
					.addOnFailureListener { e ->
						make(
							registerActivityLayout,
							"Sorry! There was a problem while creating your profile",
							LENGTH_LONG
						).show()
					...
					Toast.makeText(
						this,
						"Account created! Welcome to Art My Breath",
						Toast.LENGTH_SHORT
					).show()
					firebaseAuth.signOut()
				...
				make(
					registerActivityLayout,
					"Sorry! There was a problem while creating your account",
					LENGTH_LONG
				).show()
				hideLoadingAlert()
